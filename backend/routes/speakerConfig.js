...
router.get("/", async (req, res) => {
  try {
    const dbResult = await db.execute(
      "SELECT config_json FROM registration_page_config_speaker WHERE page = ? LIMIT 1",
      ["speaker"]
    );
    // Defensive: handle both [rows, fields] and direct array
...
// POST update speaker config
router.post("/config", async (req, res) => {
  const config = req.body;
  try {
    await db.execute(
      "UPDATE registration_page_config_speaker SET config_json = ? WHERE page = ?",
      [JSON.stringify(config), "speaker"]
    );
...
// DELETE speaker config
router.delete("/", async (req, res) => {
  try {
    await db.execute(
      "DELETE FROM registration_page_config_speaker WHERE page = ?",
      ["speaker"]
    );
    res.json({ success: true, message: "Speaker config deleted." });
  } catch (err) {
