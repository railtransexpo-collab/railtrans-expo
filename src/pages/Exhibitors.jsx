...
async function fetchConfig() {
  const res = await fetch("http://localhost:5000/api/exhibitor-config");
...
export default function Exhibitors() {
  ...
  useEffect(() => {
    ...
    fetchConfig()
      .then((cfg) => {
        if (mounted) {
          setConfig(cfg || {}); // fallback empty object
          setLoading(false);
        }
      })
      .catch((e) => {
  ...
  async function handleContinue() {
    ...
    if (step === 1) {
      try {
        const res = await fetch("http://localhost:5000/api/exhibitors", {
          method: "POST",
